...
@router.post(
    "/{username}/quizProgress/{quiz_id}", response_model=QuizProgress, status_code=201
)
async def upload_quiz_progress(username: str, quiz_id: str, progress: QuizProgress):
    """Upload quiz progress for a user."""
    return await QuizService.upload_quiz_progress(username, quiz_id, progress)


@router.get("/{username}/quizProgress", response_model=List[QuizProgress])
async def list_quiz_progress(username: str):
    """Get all quiz progress for a user."""
    return await QuizService.list_quiz_progress(username)
